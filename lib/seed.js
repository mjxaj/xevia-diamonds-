...
// Inject the gallery + lifestyle-first primary image (designs map 1:1 by index)
// Primary uses the -3 lifestyle shot (cream/silk surfaces, more luxe);
// carousel order: lifestyle → close-up → model
PRODUCTS.forEach((p, i) => {
  const n = i + 1;
  p.primary_image = `/images/products/design-${n}-3.jpg`;
  p.gallery = JSON.stringify([3, 1, 2].map((k) => `/images/products/design-${n}-${k}.jpg`));
});
